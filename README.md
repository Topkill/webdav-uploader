...
- **主页**：当前连接信息、选择文件上传、进度
- **更多 → 设置**：WebDAV 连接、超时、保活
- **更多 → 上传记录**：按任务/批次列表（默认），点进看完整明细；无任务数上限，可删除
...
点「应用已勾选保活」会按勾选项申请系统权限或跳转设置。

## 大批量文件

- 上传串行处理，逐个解析 metadata，不把全部文件内容读入内存
- 单次选择上限约 2000 个 Uri（系统 Intent/ClipData 限制）
- 日志自动截断，避免几千条日志撑爆内存
...
- 每次上传 = 一个任务
- 列表显示：时间、共/成功/失败/取消
- 点进任务：搜索、筛选成功/失败、折叠详情
- **无任务数量上限**，完整结果落盘到本地；用户可删除单个任务或清空

## 大批量 Uri

- 选中的文件列表放在**进程内队列**，不再依赖 ClipData 只挂前 500 个权限
- 同一进程内 Activity → Service 完整传递
